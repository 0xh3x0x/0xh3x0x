...
ROWS = [
    ("host",),
    ("kv", "Now", "Offensive Security Professional"),
    ("kv", "Role", "Web, API, Active Directory & Cloud Security"),
    ("kv", "Cert", "OSCP, eJPT"),
    ("kv", "Edu", "BCA Graduate"),
    ("gap",),

    ("sec", "Stack"),
    ("kv", "Languages", "Python, Bash, PowerShell, C/C++"),
    ("kv", "Security", "Burp Suite, Nuclei, SQLMap, Metasploit, BloodHound"),
    ("kv", "Cloud", "Azure, Cloudflare, Wazuh, Suricata"),
    ("kv", "Dev", "Git, Docker, Linux, Playwright"),
    ("gap",),

    ("sec", "Highlights"),
    ("bul", "OSCP Certified Offensive Security Professional"),
    ("bul", "Specialized in Web, API & Active Directory Pentesting"),
    ("bul", "Built Python automation tools for security & reporting"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">0xh3x0x@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">-0xh3x0x</tspan><tspan fill="{MUTED}">@</tspan>'
