...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "0xh3x-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">0xh3x0x@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'0xh3x0x@github:~$ whoami <tspan fill="{INK}">Kamlesh Kathiriya</tspan></text>')
